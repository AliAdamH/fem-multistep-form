...
function AddOns({ changeData, defaultValues, selectedAddons, planFrequency }) {
  ...
  const renderClassesIfChecked = (addOn) => {
    return isChecked(addOn)
      ? 'bg-neutral-magnolia border-primary-purplish'
      : '';
  };
        ...
        {Object.keys(defaultValues.addOns).map((addOn) => {
          ...
          let displayedPrice =
            planFrequency === 'monthly'
              ? `+$${addOnToRender.monthly}/mo`
              : `+$${addOnToRender.yearly}/y`;

          return (
            <div
              key={addOn}
              className={`${renderClassesIfChecked(
                addOn
              )} p-3 md:p-4 border border-neutral-lightgray rounded-lg flex md:gap-10 items-center hover:border-primary-purplish`}
            >
              <input
                onChange={() => updateSelectedAddOns(addOn, addOnToRender)}
                type="checkbox"
                name={addOn}
                id={addOn}
                checked={isChecked(addOn)}
                className="w-4 h-4 mr-4 md:mr-0"
              />
              <label
                htmlFor={addOn}
                className="text-primary-marine font-semibold"
              >
                {addOnToRender.label} <br />
                <span className="text-sm text-neutral-coolgray font-normal">
                  {addOnToRender.details}
                </span>
              </label>
              <span className="inline-block ml-auto text-primary-purplish  text-sm font-semibold">
                {displayedPrice}
              </span>
            </div>
          );
