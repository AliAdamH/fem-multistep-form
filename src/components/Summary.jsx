...
function Summary({ selectedPlan, planFrequency, selectedAddons, changePlan }) {
  ...
  const TotalPrice = () => {
    return [selectedPlan, ...Object.values(selectedAddons)].reduce(
  ...
  const displayPriceHelper = (pricedItem) => {
    return planFrequency === 'monthly'
      ? `$${pricedItem.monthly}/mo`
      : `$${pricedItem.yearly}/yr`;
  };
  ...
  return (
    <>
      ...
      <div data-summary className="bg-neutral-magnolia p-5 rounded space-y-4">
        <div
          data-plan-summary
          className="border-b-2 border-neutral-lightgray flex items-center pb-6"
        >
          <div>
          ...
          {Object.keys(selectedAddons).map((addOn) => {
            return (
              ...
              >
                ...
                <span className="text-primary-marine">
                  {' '}
                  +{displayPriceHelper(selectedAddons[addOn])}
                </span>
              </div>
            );
          })}
      ...
      <div
        data-total
        className="flex justify-between items-center mt-8 px-4 md:px-6"
      >
        <span className="text-sm text-neutral-coolgray">
          Total (per {periodFormatter})
        </span>
        <span className="md:text-lg text-primary-purplish font-bold capitalize">
          ${TotalPrice()}/{periodFormatter}
        </span>
      </div>
    </>
  );
