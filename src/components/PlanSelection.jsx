...
function PlanSelection({ changeData, planFrequency, defaultValues, selectedPlan }) {
  ...
  return (
    ...
    <div id="header" className='mb-4 md:mb-8 space-y-1'>
        <h2 className='text-2xl md:text-3xl font-bold text-primary-marine'>Select your plan</h2>
        <p className='text-neutral-coolgray'>You have the option of monthly or yearly billing.</p>
    </div>

    <div id="fields">
      <div id="radios" className='flex flex-col md:flex-row gap-3 md:gap-4 justify-between'>
        ...
        return (<div key={planType} data-radio-field className='relative rounded-xl border border-neutral-lightgray hover:border-primary-purplish md:w-1/3 p-4'>
          ...
          <div data-radio-field-details className='flex md:flex-col gap-4 md:gap-12'>
            ...
            <div className='flex flex-col'>
              ...
              <span className='text-sm text-neutral-coolgray'>{planIsMontlhy ? `$${planObject.monthly}/mo` : `$${planObject.yearly}/y`}</span>
              {!planIsMontlhy && <span className='text-sm text-primary-marine'>2 months free</span>}
            </div>
          </div>
